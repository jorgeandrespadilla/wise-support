...
export const verifyAccessToken = (token: string) => verifyToken<TokenData>(token, JWT_CONFIG.accessToken.secret);

export const verifyRefreshToken = (token: string) => verifyToken<TokenData>(token, JWT_CONFIG.refreshToken.secret);

const generateToken = <T extends object>(payload: T, secret: string, options?: SignOptions) => {
    return jwt.sign(payload, secret, options);
};

const verifyToken = <T = Record<string, any>>(token: string, secret: string): T & jwt.JwtPayload => {
    try {
